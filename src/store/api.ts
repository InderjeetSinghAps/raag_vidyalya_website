  ...
  endpoints: (builder) => ({
    ...
    >({
      ...
      transformResponse: (response) => {
        ...
        if (!result)
          throw new Error(
            'Verify OTP response missing token or user',
          );
        return {
          ...result,
    ...
    >({
      query: (body) => ({
        ...
        body: {
          ...body,
          deviceType: body.deviceType ?? DEVICE_TYPE,
          deviceToken: body.deviceToken ?? 'NA',
        },
      }),
      transformResponse: (response) => {
        const result = extractAuthResponse(
    ...
    getUserDetail: builder.query<User, void>({
      ...
      transformResponse: (response: UserDetailApiResponse) =>
        response.user,
      providesTags: ['User'],
    }),
      ...
      async onQueryStarted(
        ...
        try {
          ...
          const result = await dispatch(
            api.endpoints.getUserDetail.initiate(undefined, {
              forceRefetch: true,
            }),
          );
    ...
    getRaags: builder.query<RaagsApiResponse, void>({
      ...
      transformResponse: (response: RaagsApiResponse) => ({
        ...
        raags: response.raags.map((r) => ({
          ...r,
          audioUrl: r.audioUrl
            ? getGoogleDriveAudioUrl(r.audioUrl)
            : '',
          listOfBandish: r.listOfBandish.map((b) => ({
            ...b,
            audioUrl: b.audioUrl
              ? getGoogleDriveAudioUrl(b.audioUrl)
              : null,
          })),
        })),
      }),
    ...
    getPreviousResults: builder.query<
      import('@/types').PreviousResultsApiResponse,
      void
    >({
    ...
    >({
      ...
      transformResponse: (response: {
        success: boolean;
        message: string;
        collaborator: import('@/types').Collaborator;
      }) => ({
    ...
    checkDevice: builder.mutation<
      import('@/types').CheckDeviceResponse,
      { deviceId: string }
    >({
      query: ({ deviceId }) => ({
    ...
    getMyReferralCode: builder.query<
      import('@/types').ReferralCodeResponse,
      void
    >({
    ...
    >({
      ...
      transformResponse: (
        response: import('@/types').ReferralHistoryResponse,
      ) => ({
        ...response,
    ...
    getRaagAccess: builder.query<
      import('@/types').RaagAccessResponse,
      void
    >({
    ...
    getSingleRaagAccess: builder.query<
      import('@/types').SingleRaagAccessResponse,
      number
    >({
      query: (raagNumber) => `/referral/raag-access/${raagNumber}`,
      providesTags: (_result, _error, raagNumber) => [
        { type: 'Referral', id: raagNumber },
      ],
    }),
  }),
});
