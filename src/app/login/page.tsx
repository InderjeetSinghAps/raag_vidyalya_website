...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    try {
      const response = await authService.login(data.email, data.password, "NA", 2)
      ...
      rightPanelContent={
        ...
        >
          <p
            ...
            style={{ color: "#6B5E4A" }}
          >
            "Begin your journey into the divine world of Gurmat Sangeet."
          </p>
        </motion.div>
      }
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                className="h-4 w-4 rounded border-[#D4A44A]/40 accent-[#D4A44A]"
              />
              <span
            ...
            <Link
              ...
              className="font-sans text-xs font-semibold text-[#D4A44A] transition-colors hover:text-[#B8860B]"
            >
              Forgot password?
            </Link>
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="h-px flex-1 bg-[#D4C4A0]" />
          <span
        ...
        <Button
          variant="outline"
          className="h-11 w-full rounded-xl border-[#D4C4A0] bg-white text-sm font-medium text-[#0B1220] shadow-sm transition-all duration-200 hover:border-[#D4A44A]/50 hover:bg-[#FFFDF8] hover:shadow-md active:scale-[0.97]"
        >
