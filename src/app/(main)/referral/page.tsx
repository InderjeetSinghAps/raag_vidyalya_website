...
import { useGetMyReferralCodeQuery, useGetReferralHistoryQuery } from "@/store/api"
...
const statCards = [
  { key: "total" as const, label: "Total", icon: Users, text: "text-blue-400" },
  { key: "successful" as const, label: "Success", icon: CheckCircle, text: "text-green-400" },
  { key: "pending" as const, label: "Pending", icon: Clock, text: "text-yellow-400" },
  { key: "failed" as const, label: "Failed", icon: XCircle, text: "text-red-400" },
]

const statusConfig: Record<string, { label: string; classes: string; border: string }> = {
  pending: { label: "Pending", classes: "border-yellow-500/30 bg-yellow-500/10 text-yellow-400", border: "border-l-yellow-500" },
  successful: { label: "Successful", classes: "border-green-500/30 bg-green-500/10 text-green-400", border: "border-l-green-500" },
  failed: { label: "Failed", classes: "border-red-500/30 bg-red-500/10 text-red-400", border: "border-l-red-500" },
}
...
export default function ReferralPage() {
  ...
  const { data: referralData, isLoading: referralLoading } = useGetMyReferralCodeQuery(undefined)
  const { data: historyData, isLoading: historyLoading } = useGetReferralHistoryQuery({ page, limit: 20 })

  const stats = historyData?.stats as ReferralHistoryStats | undefined
    ...
    <div className="mx-auto max-w-[760px] px-4 py-8">
      ...
      <div className="mb-10 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Share your code and earn rewards</p>
        </div>
      </div>
        ...
        <div className="mb-10 rounded-xl border border-border/50 bg-card/60 p-8 backdrop-blur-sm shadow-sm dark:border-white/[0.06] dark:bg-white/[0.03]">
          <div className="flex justify-around">
            {statCards.map((card) => {
              ...
              return (
                <div key={card.key} className="flex flex-col items-center gap-1.5">
                  <Icon className={`size-4 ${card.text}`} />
                  <p className={`text-2xl font-bold ${card.text}`}>{stats[card.key]}</p>
          ...
          <div className="mt-6 border-t border-border/40 pt-5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground/60">{Math.round((stats.raagUnlockCount / TOTAL_UNLOCKABLE) * 100)}%</span>
            </div>
            {stats.allRaagsUnlocked ? (
              <p className="mt-1 text-xs text-green-500">All raags unlocked!</p>
            ) : (
